...
def main(target_layers: tuple[str, ...]):
    ...
    os.makedirs(file_path / "traj" / "incremental_3DVar" / f"{bec_source}", exist_ok=True)

    np.savez(file_path / "traj" / "incremental_3DVar" / f"{bec_source}" / f"on_{prefix}.npz", X=X_traj, Y=Y_traj, Z=Z_traj)
