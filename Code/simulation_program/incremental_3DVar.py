...
def main(target_layers: tuple[str, ...]):
    ...
    np.savez(file_path / "traj" / f"incremental_3DVar_on_{prefix}_{bec_source}.npz", X=X_traj, Y=Y_traj, Z=Z_traj)
    np.savez(file_path / "rmse" / f"incremental_3DVar_on_{prefix}_{bec_source}.npz", X=X_RMSE, Y=Y_RMSE, Z=Z_RMSE)
